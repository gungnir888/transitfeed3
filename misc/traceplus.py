...
def make_expanded_trace(frame_records):
    """Return a list of text lines for the given list of frame records."""
    dump = []
    for (frame_obj, filename, line_num, fun_name, context_lines, context_index) in frame_records:
...
def run_with_expanded_trace(closure):
    try:
        return closure()
        ...
        dashes = '%s\n' % ('-' * 60)
        dump = [dashes]
        dump.extend(make_expanded_trace(frame_records))
