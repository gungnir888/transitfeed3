...
class SqlLoop(cmd.Cmd):
    ...
    def __init__(self, cursor):
        ...
        self.cursor = cursor
        self.doc = r"""
        List stops by trip in order visited:
        SELECT trip_headsign,trip_id,stop_name from stop_times NATURAL JOIN trips
        NATURAL JOIN stops WHERE trip_headsign LIKE '%intern%' ORDER BY trip_id,time
        """
...
def load_named_file(file_name, conn):
    basename = os.path.basename(file_name)
    last_dot = basename.rfind(".")
    if last_dot > 0:
        basename = basename[:last_dot]
    file_object = open(file_name, "rb")
    load_file(file_object, basename, conn)


def load_file(f, table_name, conn):
...
def main():
    ...
    parser.add_option(
        '-f', '--load_file', dest='load_file', metavar='FILE', help='Load FILE into the db'
    )
    parser.add_option(
        '-d', '--load_dir', dest='load_dir', metavar='FILE', help='Load FILE/*.txt into the db'
    )
    parser.add_option(
        '-i', '--interactive', dest='interactive', action='store_true', help='Go into command prompt mode'
    )
    parser.add_option('', '--db', dest='database', metavar='FILE', help='sqlite db')
    ...
    if options.load_dir:
        for entry in os.listdir(options.load_dir):
            if re.search(r"\.txt", entry):
                load_named_file(os.path.join(options.load_dir, entry), conn)

    if options.load_file:
        load_named_file(options.load_file, conn)
