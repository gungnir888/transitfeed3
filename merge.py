...
class HTMLProblemAccumulator(transitfeed.ProblemAccumulatorInterface):
    ...
    @staticmethod
    def _generate_stats_table(feed_merger):
        """
        ...
        """
        rows = ['<tr><th class="header"/><th class="header">Merged</th>'
...
class DataSetMerger:
    ...
    @staticmethod
    def _merge_identical_case_insensitive(a, b):
    ...
    def _merge_same_agency(self, a_agency_id, b_agency_id):
        ...
        a_agency = self.feed_merger.a_schedule.get_agency(a_agency_id)._migrated_entity
        b_agency = self.feed_merger.b_schedule.get_agency(b_agency_id)._migrated_entity
    ...
    def _merge_same_id(self):
        ...
        for a in self._get_iter(self.feed_merger.a_schedule):
            ...
            try:
                ...
                self._report_same_id_but_not_merged(self._get_id(a), merge_error)

        for b in self._get_iter(self.feed_merger.b_schedule):
            try:
                self._get_by_id(self.feed_merger.a_schedule, self._get_id(b))
            except KeyError:
        ...
        for a in a_not_merged:
            new_id = self._has_id(self.feed_merger.b_schedule, self._get_id(a))
            self._add(a, None, self._migrate(a, self.feed_merger.a_schedule, new_id))
        for b in b_not_merged:
            new_id = self._has_id(self.feed_merger.a_schedule, self._get_id(b))
            self._add(None, b, self._migrate(b, self.feed_merger.b_schedule, new_id))
    ...
    def _merge_by_id_keep_new(self):
        ...
        for orig in self._get_iter(self.feed_merger.a_schedule):
            migrated = self._migrate(orig, self.feed_merger.a_schedule, True)
            a_orig_migrated[self._get_id(migrated)] = orig, migrated

        for orig in self._get_iter(self.feed_merger.b_schedule):
            migrated = self._migrate(orig, self.feed_merger.b_schedule, True)
    ...
    def _merge_different_id(self):
        ...
        for a in self._get_iter(self.feed_merger.a_schedule):
            if a not in self.feed_merger.a_merge_map:
                ...
                new_id = self._has_id(self.feed_merger.b_schedule, self._get_id(a))
                self._add(a, None,
                          self._migrate(a, self.feed_merger.a_schedule, new_id))
        for b in self._get_iter(self.feed_merger.b_schedule):
            if b not in self.feed_merger.b_merge_map:
                self._num_not_merged_b += 1
                new_id = self._has_id(self.feed_merger.a_schedule, self._get_id(b))
                self._add(None, b,
                          self._migrate(b, self.feed_merger.b_schedule, new_id))

        return self._num_merged

    def _report_same_id_but_not_merged(self, entity_id, reason):
        ...
        """
        self.feed_merger.problem_reporter.same_id_but_not_merged(self, entity_id, reason)
    ...
    def _migrate(self, entity, schedule, new_id):
        ...
        Args:
          ...
          new_id: Whether to generate a new id (True) or keep the original (False).
...
class StopMerger(DataSetMerger):
    ...
    def merge_data_sets(self):
        ...
        self._update_and_migrate_unmerged(self._a_not_merged, fm.a_zone_map, fm.a_merge_map, fm.a_schedule)
        self._update_and_migrate_unmerged(self._b_not_merged, fm.b_zone_map, fm.b_merge_map, fm.b_schedule)
    ...
    def _update_and_migrate_unmerged(self, not_merged_stops, zone_map, merge_map, schedule):
...
class ServicePeriodMerger(DataSetMerger):
    ...
    def _migrate(self, original_service_period, schedule, new_id):
...
class FareMerger(DataSetMerger):
    ...
    def _migrate(self, original_fare, schedule, new_id):
...
class TransferMerger(DataSetMerger):
    ...
    def _migrate(self, original_transfer, schedule, new_id=True):
...
class ShapeMerger(DataSetMerger):
    ...
    def _migrate(self, original_shape, schedule, new_id):
...
class TripMerger(DataSetMerger):
    ...
    def _report_same_id_but_not_merged(self, trip_id, reason):
        pass
    ...
    def _migrate(self, original_trip, schedule, new_id):
...
class FeedMerger(object):
    ...
    @staticmethod
    def _find_largest_id_postfix_number(schedule):
        ...
        def extract_postfix_number(e_id):
            ...
            if e_id is None:
                return 0
            match = postfix_number_re.search(e_id)
