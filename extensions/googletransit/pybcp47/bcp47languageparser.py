...
class Bcp47LanguageParser:
    ...
    @staticmethod
    def _get_lines_from_language_subtag_registry_file():
    ...
    def _add_subtag_from_registry_file(
        ...
        if '..' in current_tag:
            ...
            for i in range(
                    self.int_str26to_int(start_str), self.int_str26to_int(end_str) + 1):
                range_tag = self.int_to_int_str26(i)
                self._add_subtag_from_registry_file(
                    current_type, range_tag,
                    current_descriptions, current_prefixes,
                    line_number
                )
        ...
        if current_type == 'language':
            ...
            raise FileParseError(line_number, "Invalid Type %s for Subtag %s" % (current_type, current_tag))

    # A dictionary for the regular expression strings to test wellformedness.
    _wellformed_dict = {'iana_lang': "[a-zA-Z]{4,8}", 'extlang': "[a-zA-Z]{3}"}
    ...
    _wellformed_dict['iso_lang_extlang'] = "[a-zA-Z]{2,3}(-%(extlang)s){0,3}" % _wellformed_dict
    _wellformed_dict['lang'] = "(%(iana_lang)s|%(iso_lang_extlang)s|x)" % _wellformed_dict
    ...
    def is_well_formed(self, lang_code):
