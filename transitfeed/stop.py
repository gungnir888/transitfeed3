...
class Stop(GtfsObjectBase):
    ...
    def _get_trip_index(self, schedule=None):
        ...
        trip: a Trip object
        index: an offset in trip.get_stop_times()
        """
        trip_index = []
        for trip, sequence in self._get_trip_sequence(schedule):
            for index, st in enumerate(trip.get_stop_times()):
    ...
    def get_stop_time_trips(self, schedule=None):
        ...
        trip: a Trip object.
        index: the offset of this stop in trip.get_stop_times(), which may be
          different from the stop_sequence.
        is_timepoint: a bool
        """
        time_trips = []
        for trip, index in self._get_trip_index(schedule):
            secs, stoptime, is_timepoint = trip.get_time_interpolated_stops()[index]
    ...
    def validate_stop_url(self, problems):
        value = self.stop_url
        if value and not util.validate_url(value, 'stop_url', problems):
            del self.stop_url
    ...
    def validate_stop_timezone(self, problems):
        ...
        # stop_timezone.
        util.validate_timezone(self.stop_timezone, 'stop_timezone', problems)
