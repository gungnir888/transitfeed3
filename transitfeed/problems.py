...
class ProblemReporter:
    ...
    def add_to_accumulator(self, e):
        ...
        self.accumulator.report(e)

    def new_version_available(self, version):
        e = NewVersionAvailable(version=version, problem_type=TYPE_NOTICE,
                                url='https://github.com/google/transitfeed')
        self.add_to_accumulator(e)

    def feed_not_found(self, feed_name, context=None, problem_type=TYPE_ERROR):
        e = FeedNotFound(feed_name=feed_name, context=context,
                         context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def unknown_format(self, feed_name, context=None, problem_type=TYPE_ERROR):
        e = UnknownFormat(feed_name=feed_name, context=context,
                          context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def file_format(self, problem, context=None, problem_type=TYPE_ERROR):
        e = FileFormat(problem=problem, context=context,
                       context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def missing_file(self, file_name, context=None, problem_type=TYPE_ERROR):
        e = MissingFile(file_name=file_name, context=context,
                        context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def unknown_file(self, file_name, context=None, problem_type=TYPE_WARNING):
        e = UnknownFile(file_name=file_name, context=context,
                        context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def empty_file(self, file_name, context=None, problem_type=TYPE_ERROR):
    ...
    def unrecognized_column(self, file_name, column_name, context=None,
        ...
        e = UnrecognizedColumn(file_name=file_name, column_name=column_name,
                               context=context, context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)
    ...
    def csv_syntax(self, description=None, context=None, problem_type=TYPE_ERROR):
        e = CsvSyntax(description=description, context=context,
                      context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def duplicate_column(self, file_name, header, count, problem_type=TYPE_ERROR,
                         context=None):
    ...
    def duplicate_id(self, column_names, values, context=None, problem_type=TYPE_ERROR):
    ...
    def invalid_agency_id(self, column_name, value, relating_type, relating_id,
                          context=None, problem_type=TYPE_ERROR):
        e = InvalidAgencyID(column_name=column_name, value=value,
                            relating_problem_type=relating_type, relating_id=relating_id,
                            context=context, context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def unused_stop(self, stop_id, stop_name, context=None, problem_type=TYPE_WARNING):
        e = UnusedStop(stop_id=stop_id, stop_name=stop_name,
                       context=context, context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def unused_station(self, stop_id, stop_name, context=None, problem_type=TYPE_ERROR):
    ...
    def stop_too_far_from_parent_station(self, stop_id, stop_name, parent_stop_id,
                                         ...
                                         problem_type=TYPE_WARNING, context=None):
    ...
    def stops_too_close(self, stop_name_a, stop_id_a, stop_name_b, stop_id_b,
                        distance, problem_type=TYPE_WARNING, context=None):
        e = StopsTooClose(
    ...
    def different_station_too_close(self, stop_name, stop_id,
        ...
        e = DifferentStationTooClose(
            ...
            distance=distance, context=context, context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)
    ...
    def stop_too_far_from_shape_with_dist_traveled(self, trip_id, stop_name, stop_id,
        ...
        e = StopTooFarFromShapeWithDistTraveled(
            ...
            distance=distance, max_distance=max_distance, problem_type=problem_type)
        self.add_to_accumulator(e)
    ...
    def no_service_exceptions(self, start, end, problem_type=TYPE_WARNING, context=None):
        e = NoServiceExceptions(start=start, end=end, context=context,
                                context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def invalid_line_end(self, bad_line_end, context=None, problem_type=TYPE_WARNING):
        """bad_line_end is a human readable string."""
        e = InvalidLineEnd(bad_line_end=bad_line_end, context=context,
                           context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def too_fast_travel(self, trip_id, prev_stop, next_stop, dist, time_travelled, speed,
                        problem_type=TYPE_ERROR):
        e = TooFastTravel(trip_id=trip_id, prev_stop=prev_stop,
                          next_stop=next_stop, time_travelled=time_travelled, dist=dist, speed=speed,
                          context=None, context2=self._context, problem_type=problem_type)
        self.add_to_accumulator(e)

    def stop_with_multiple_route_types(self, stop_name, stop_id, route_id1, route_id2,
                                       context=None, problem_type=TYPE_WARNING):
    ...
    def other_problem(self, description, context=None, problem_type=TYPE_ERROR):
    ...
    def minimum_transfer_time_set_with_invalid_transfer_type(self,
        ...
        e = MinimumTransferTimeSetWithInvalidTransferType(context=context,
                                                          context2=self._context, transfer_problem_type=transfer_type,
                                                          problem_type=problem_type)
        self.add_to_accumulator(e)
    ...
    def too_many_consecutive_stop_times_with_same_time(self,
        ...
        e = TooManyConsecutiveStopTimesWithSameTime(trip_id=trip_id,
                                                    ...
                                                    stop_time=util.format_seconds_since_midnight(time_in_secs),
                                                    context=None,
                                                    context2=self._context,
                                                    problem_type=problem_type)
        self.add_to_accumulator(e)
...
class ProblemAccumulatorInterface:
...
class SimpleProblemAccumulator(ProblemAccumulatorInterface):
    ...
    def report(self, e):
        ...
        if context:
            print(context)
        print(self._line_wrap(e.format_problem(), 78))

    @staticmethod
    def _line_wrap(text, size):
        """
        ...
        """
        return reduce(lambda line, word, width=size: '%s%s%s' % (
            line,
            ' \n'[(len(line) - line.rfind('\n') - 1 + len(word.split('\n', 1)[0]) >= size)],
            word
        ), text.split(' '))
...
class ExceptionWithContext(Exception):
    ...
    def get_dict_to_format(self):
        ...
        for k, v in self.__dict__.items():
            ...
            # Concatenating a unicode and utf-8 str object causes an exception such
...
class StopTooFarFromParentStation(ExceptionWithContext):
    ...
    def __cmp__(self, y):
        ...
        return util.cmp(y.distance, self.distance)
...
class StopsTooClose(ExceptionWithContext):
    ...
    def __cmp__(self, y):
        ...
        return util.cmp(self.distance, y.distance)
...
class TooFastTravel(ExceptionWithContext):
    ...
    def __cmp__(self, y):
        ...
        return util.cmp(y.dist, self.dist)
