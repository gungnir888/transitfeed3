...
def check_version(problems, latest_version=None):
    """
    ...
    if not latest_version:
        ...
        try:
            ...
            description = ('During the new-version check, we failed to reach '
                           'transitfeed server: Reason: %s.' % he)
...
def _max_version(versions):
    ...
    if len(versions) == 0:
        return None
    return max(versions, key=lambda x: tuple(int(item) for item in x.split('.')))
...
def validate_url(url, column_name=None, problems=None):
    """
    Validates a non-required URL value using is_valid_url():
    ...
    """
    if is_empty(url) or is_valid_url(url):
        return True
    else:
        if problems:
...
def validate_email(email, column_name=None, problems=None):
    """
    ...
    """
    if is_empty(email) or re.match(r'[^@]+@[^@]+\.[^@]+', email):
        return True
    else:
        if problems:
...
def validate_language_code(lang, column_name=None, problems=None):
    ...
    """
    if is_empty(lang) or is_valid_language_code(lang):
        return True
    else:
        if problems:
...
def validate_timezone(timezone, column_name=None, problems=None):
    ...
    Validates a non-required timezone string value using is_valid_timezone():
      ...
      - an empty timezone string is regarded as valid! Otherwise we might end up
        with many duplicate errors because of the required field checks.
    """
    if is_empty(timezone) or is_valid_timezone(timezone):
        return True
    else:
        if problems:
...
def is_valid_date(date):
    ...
    """
    return date_string_to_date_object(date) is not None
...
def validate_date(date, column_name=None, problems=None):
    ...
    Validates a non-required date string value using is_valid_date():
      ...
      - an empty date string is regarded as valid! Otherwise we might end up
        with many duplicate errors because of the required field checks.
    """
    if is_empty(date) or is_valid_date(date):
        return True
    else:
        if problems:
...
def validate_required_fields_are_not_empty(gtfs_object, required_field_names,
                                           problems=None):
    """
...
def validate_and_return_int_value(value, allowed_values, default, allow_empty,
    ...
    Validates a value to be a valid integer in the list of allowed values:
      ...
      - if integer but not in allowed_values adds invalid_value warning
    ...
    Args:
      ...
      default: a default value for invalid string values
    ...
    Returns:
      An integer value.
    """
    if allow_empty and is_empty(value):
        return default
    try:
        int_value = int(value)
...
def color_luminance(color):
    ...
    Args:
      color: a string of 6 hex digits in the format verified by is_valid_hex_color().
...
def is_valid_yes_no_unknown(value):
    return value in ['0', '1', '2']


def validate_yes_no_unknown(value, column_name=None, problems=None):
    """Validates a value "0" for unknown, "1" for yes, and "2" for no."""
    if is_empty(value) or is_valid_yes_no_unknown(value):
        return True
    else:
        if problems:
            problems.invalid_value(column_name, value)
...
def format_seconds_since_midnight(s):
...
def float_string_to_float(float_string, problems=None):
    ...
    if not match and problems is not None:
        ...
        problems.invalid_float_value(float_string)
    return parsed_value


def non_neg_int_string_to_int(int_string, problems=None):
    ...
    if parsed_value < 0:
        ...
        problems.invalid_non_negative_integer_value(int_string)
...
def approximate_distance(degree_lat1, degree_lng1, degree_lat2, degree_lng2):
...
def approximate_distance_between_stops(stop1, stop2):
    ...
    if (stop1.stop_lat is None or stop1.stop_lon is None or
        ...
        return None
    return approximate_distance(stop1.stop_lat, stop1.stop_lon,
...
class EndOfLineChecker:
    ...
    def __next__(self):
        ...
        if m_eol.group() == "\x0d\x0a":
            ...
            self._problems.invalid_line_end(
        ...
        if self._crlf > 0 and self._lf > 0:
            ...
            self._problems.other_problem(
                ...
                "consistent line end." % (self._crlf, crlf_plural, crlf_plural,
                                          ...
                                          lf_plural, lf_lines),
                (self._name,))
            # Prevent _final_check() from reporting the problem twice, in the unlikely
...
class ISO639(object):
    ...
    codes_2letter = {'aa', 'ab', 'ae', 'af', 'ak', 'am', 'an', 'ar', 'as', 'av', 'ay', 'az', 'ba', 'be', 'bg', 'bh',
                     ...
                     'pi', 'pl', 'ps', 'pt', 'qu', 'rm', 'rn', 'ro', 'ru', 'rw', 'sa', 'sc', 'sd', 'se', 'sg', 'si',
