...
class ScheduleServer(BaseServer):
    key = None
    schedule = None
    file_dir = None
    host = None
...
class ScheduleRequestHandler(BaseHTTPRequestHandler):
    ...
    def handle_json_GET_routerow(self, params):
        schedule = self.server.schedule
        route = schedule.get_route(params.get('route', None))
        return [transitfeed.Route.FIELD_NAMES, route.get_field_values_tuple()]
    ...
    def handle_json_GET_tripstoptimes(self, params):
        ...
        for arr, dep, stop in time_stops:
            stops.append(stop_to_tuple(stop))
    ...
    def handle_json_GET_neareststops(self, params):
        ...
        return [stop_to_tuple(s) for s in stops]
    ...
    def handle_json_GET_stopsearch(self, params):
        ...
        for s in schedule.get_stop_list():
            if s.stop_id.lower().find(query) != -1 or s.stop_name.lower().find(query) != -1:
                matches.append(stop_to_tuple(s))
        return matches
...
def main(request_handler_class=ScheduleRequestHandler):
    ...
    server = StoppableHTTPServer(server_address=('', options.port), RequestHandlerClass=request_handler_class)
    ...
    print("To view, point your browser at http://localhost:%d/" % server.server_port)
